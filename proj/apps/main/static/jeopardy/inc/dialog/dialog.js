...
var SWANK_DIALOG_CONFIG =
{
	 ...
	 * ---------------------------------------------------------------------------------------------
	 * By default, the internal 'Button' class is used.  But you can override this, say with the 
...
function Dialog()
{
	...
	*/
	...
	me.Launch = function(Settings)
	{
		...
		{
			let classContent = [SWANK_DIALOG_CONFIG.ButtonClass, `Button${i}`];
			let styleContent = [`min-width: 50px`];

			if ('Class' in Settings.Buttons[i]) classContent.push(Settings.Buttons[i].Class);
			if ('Style' in Settings.Buttons[i]) classContent.push(Settings.Buttons[i].Style);
			if ('Width' in Settings.Buttons[i]) styleContent.push(`width: ${Settings.Buttons[i].Width}px`);
			
			let buttonHTML = [];
			buttonHTML.push(`<a`)
			if (classContent.length > 0) 
			{
				let addHTML = classContent.join(' ');
				buttonHTML.push(`class="${addHTML}"`)
			}
			if (styleContent.length > 0) 
			{
				let addHTML = styleContent.join('; ');
				buttonHTML.push(`style="${addHTML};"`)
			}
			buttonHTML.push(`href="javascript:void(0)">${Settings.Buttons[i].Name}</a>`);
			ButtonsHTML.push(buttonHTML.join(' '));
		}
	...
	me.Info = function(Arg)
	{
		...
		Arg.Icon = 'info';
		if (!('Height' in Arg)) Arg.Height = 200;
		if (!('Width' in Arg)) Arg.Width = 400;
		me.Launch(Arg);
	};
